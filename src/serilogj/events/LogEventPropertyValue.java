...
public abstract class LogEventPropertyValue {
	/** 
	 ...
	 @param format A format string applied to the value, or null.
	 @param formatProvider A format provider to apply to the value, or null to use the default.
	 {@link LogEventPropertyValue.ToString(string, IFormatProvider)} .
	*/
	...
	*/
	...
	/** 
	 ...
	 @param format The format to use.-or- A null reference (Nothing in Visual Basic) to use
	 the default format defined for the type of the <see cref="T:System.IFormattable"/> implementation. 
	 @param formatProvider The provider to use to format the value.-or- A null reference
	 (Nothing in Visual Basic) to obtain the numeric format information from the current locale
	 setting of the operating system. <filterpriority>2</filterpriority>
	*/
