...
public abstract class PeriodicBatchingSink implements ILogEventSink, Closeable {
	...
	private void execute() {
		try
		{
			...
			SelfLog.writeLine("Exception while emitting periodic batch from %s: %s", getClass().getName(), ex.getMessage());
			status.markFailure();
		} finally {
	...
	// We possibly need to do something with synchronized context but it's not supported by serilogj yet
