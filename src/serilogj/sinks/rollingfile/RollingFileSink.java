...
public class RollingFileSink implements ILogEventSink, Closeable {
	...
	public RollingFileSink(String pathFormat, Long fileSizeLimitBytes, Integer retainedFileCountLimit, boolean buffered, ITextFormatter formatter) {
		if (pathFormat == null) {
	...
	private void openFile(LocalDateTime now) {
		...
		nextCheckpoint = LocalDateTime.of(now.getYear(), now.getMonth(), now.getDayOfMonth() + 1, 0, 0);
		...
		for(int attempt = 0; attempt < maxAttempts; attempt++) {
			...
			try {
				...
				SelfLog.writeLine("Rolling file target %s was locked, attempting to open next in sequence (%d)", path, sequenceNumber);
				sequenceNumber ++;
				continue;
			}
