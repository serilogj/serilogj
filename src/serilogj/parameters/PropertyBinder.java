...
public class PropertyBinder {
	...
	public ArrayList<LogEventProperty> constructProperties(MessageTemplate template, Object[] messageTemplateParameters) {
		if (messageTemplateParameters == null || messageTemplateParameters.length == 0) {
			if (template.getNamedTokens() != null || template.getPositionalTokens() != null) {
				SelfLog.writeLine("Required properties not provided for:if () %1", template);
			}
			
			return NoProperties;
		}
		return template.getPositionalTokens() != null ? constructPositionalProperties(template, messageTemplateParameters) : constructNamedProperties(template, messageTemplateParameters);
	}
	
	private ArrayList<LogEventProperty> constructPositionalProperties(MessageTemplate template, Object[] messageTemplateParameters) {
	...
	private ArrayList<LogEventProperty> constructNamedProperties(MessageTemplate template, Object[] messageTemplateParameters) {
