...
public class PropertyValueConverter implements ILogEventPropertyValueFactory, ILogEventPropertyFactory {
	...
	public PropertyValueConverter(int maximumDestructuringDepth, java.lang.Class<?>[] additionalScalarTypes, IDestructuringPolicy[] additionalDestructuringPolicies) {
		...
		// Destructuring policies not build (I don't think they are needed in java)
	}
	...
	private LogEventPropertyValue createPropertyValue(Object value, boolean destructureObjects, int depth) {
		return createPropertyValue(value, destructureObjects ? Destructuring.Destructure : Destructuring.Default, depth);
	}
	...
	private LogEventPropertyValue createPropertyValue(Object value, Destructuring destructuring, int depth) {
		...
		if (Typ.isMap(valueType)) {
			...
			map.forEach((k, v) -> dict.put((ScalarValue)limiter.createPropertyValue(k, destructuring), limiter.createPropertyValue(v, destructuring)));
	...
	private ArrayList<LogEventProperty> getProperties(Object value, ILogEventPropertyValueFactory recursive) {
		...
		Class<?> valueType = value.getClass();
		Map<String, FieldAccess> fields = Reflection.getPropertyFieldAccessMapFieldFirstForSerializer(valueType);
		...
		for(Map.Entry<String, FieldAccess> pair : fields.entrySet()) {
			...
			result.add(new LogEventProperty(pair.getValue().alias(), recursive.createPropertyValue(propertyValue, true)));
		}
		return result;
	}
	...
	private class DepthLimiter implements ILogEventPropertyValueFactory {
		...
		public LogEventPropertyValue createPropertyValue(Object value, Destructuring destructuring) {
			...
			return (tempVar != null) ? tempVar : propertyValueConverter.createPropertyValue(value, destructuring, currentDepth + 1);
		}

		public final LogEventPropertyValue createPropertyValue(Object value, boolean destructureObjects)
		{
			LogEventPropertyValue tempVar = defaultIfMaximumDepth();
			return (tempVar != null) ? tempVar : propertyValueConverter.createPropertyValue(value, destructureObjects, currentDepth + 1);
		}
