...
public class JsonFormatter implements ITextFormatter {
	...
	/** 
	 ...
	 @param omitEnclosingObject If true, the properties of the event will be written to
	 the output without enclosing braces. Otherwise, if false, each event will be written as a well-formed
	 JSON object.
	 @param closingDelimiter A string that will be written after each log event is formatted.
	 If null, <see cref="Environment.NewLine"/> will be used. Ignored if <paramref name="omitEnclosingObject"/>
	 is true.
	 @param renderMessage If true, the message will be rendered and written to the output as a
	 property named RenderedMessage.
	 @param formatProvider Supplies culture-specific formatting information, or null.
	*/
	...
	@Override
	public void format(LogEvent logEvent, Writer output) 
			throws IOException {
		if (logEvent == null) {
		...
		if (logEvent.getException() != null) {
			writeJsonProperty("Exception", logEvent.getException(), output, true);
		}
	...
	private void writeRenderings(ArrayList<PropertyToken> tokensWithFormat,
			Map<String, LogEventPropertyValue> properties, Writer output) 
					throws IOException {
		output.write(",\"Renderings\":{");
		output.write("}");
	}

	private void writeProperties(Map<String, LogEventPropertyValue> properties, Writer output) 
			throws IOException {
	...
	private void writeJsonProperty(String name, Object value, Writer output, Boolean writeDelim) 
			throws IOException {
		if (writeDelim) {
			output.write(",");
		}
	...
	private void writeBoolean(Object value, Boolean quote, Writer output) 
			throws IOException {
		output.write((boolean)value ? "true" : "false");
	}
	
	private void writeToString(Object value, Boolean quote, Writer output) 
			throws IOException {
		if (quote) {
			output.write("\"");
		}
	...
	private void writeString(Object value, Boolean quote, Writer output) 
			throws IOException {
	...
	private void writeFloat(Object value, Boolean quote, Writer output) 
			throws IOException {
		output.write(String.format(Locale.ROOT, "%f", value));
	}
	
	private void writeLiteral(Object value, Boolean quote, Writer output) 
			throws IOException  {
		if (value == null) {
			output.write("null");
			return;
	...
	private void writeSequence(Object value, Boolean quote, Writer output) 
			throws IOException {
		output.write("[");
		String delim = "";
	...
	private void writeDictionary(Object value, Boolean quote, Writer output) 
			throws IOException {
		output.write("{");
		String delim = "";
		for(Map.Entry<ScalarValue, LogEventPropertyValue> element : ((DictionaryValue)value).getElements().entrySet()) {
			output.write(delim);
			delim = ",";
	...
	private void writeStructure(Object value, Boolean quote, Writer output) 
			throws IOException {
	...
	private void writeDate(Object value, Boolean quote, Writer output)
			throws IOException {
	...
	private void writeTemporal(Object value, Boolean quote, Writer output)
			throws IOException {
